...
fn estimate_duration_millis(source: &AudioSource, _out_channels: u32) -> i32 {
    ...
    let media_source: Box<dyn symphonia::core::io::MediaSource> = match source {
        AudioSource::Path(path) => match File::open(path) {
            Ok(file) => Box::new(file),
            Err(err) => {
                error!("Failed to open source for duration: {err}");
                return -1;
            }
        },
        AudioSource::Url(url) => {
            let response = match reqwest::blocking::get(url).and_then(|res| res.error_for_status()) {
                Ok(res) => res,
                Err(err) => {
                    error!("Failed to fetch URL for duration: {err}");
                    return -1;
                }
            };
            ...
            let bytes = match response.bytes() {
                Ok(bytes) => bytes,
                Err(err) => {
                    error!("Failed to read URL body for duration: {err}");
                    return -1;
                }
            };
        ...
        AudioSource::Memory(data) => Box::new(Cursor::new(data.clone())),
    };
